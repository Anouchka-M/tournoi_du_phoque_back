...
Router.get('/read', (req, res) => {
  let sql = 'SELECT * FROM 8eme_de_finale'

  connection.query(sql, (err, result) => {
    if (err) {
      console.error(err)
      res.status(500).send('Error retrieving 8ème_de_finale from database')
    } else {
      res.json(result)
    }
  })
...
Router.get('/livres', (req, res) => {
  let sql =
    'SELECT id_premier_livre FROM tournoi_du_phoque.8eme_de_finale WHERE votes_premier_livre > votes_second_livre UNION ALL SELECT id_second_livre FROM tournoi_du_phoque.8eme_de_finale AS id_gagnant WHERE votes_second_livre > votes_premier_livre ORDER BY id_premier_livre'

  connection.query(sql, (err, result) => {
    if (err) throw err
    let sql =
      'SELECT id_premier_livre FROM tournoi_du_phoque.8eme_de_finale UNION ALL SELECT id_second_livre FROM tournoi_du_phoque.8eme_de_finale AS id_concurrent ORDER BY id_premier_livre'
      ...
      for (let i = 0; i < idTriésTrio.length; i++) {
        ...
        connection.query(sql, values, (err, result3) => {
          if (err) {
            ...
            res
              .status(500)
              .send('Error retrieving livres 8ème de finale from database')
          } else {
            // console.log(result3)
            tableauResult.push(result3)
            if (tableauResult.length === 8) {
